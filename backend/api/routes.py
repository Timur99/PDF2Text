...
def engines() -> dict:
    return {
        ...
        "engines": [
            # Названия — по тому, что произойдёт с документом, а не по имени движка:
            # «Apple Vision» и «PaddleOCR» ничего не говорят человеку с фоткой договора.
            # По типу файла назвать нельзя: скан PDF и фотография требуют одного режима,
            # а «Для PDF» на сканированном PDF отказал бы — название обмануло бы выбор.
            {
                "id": "auto",
                "label": "Авто",
                "available": True,
                "description": "Подходит для всего. Готовый текст берётся как есть, "
            ...
            },
            {
                ...
                "label": "Готовый текст",
                "available": True,
                "description": "Только текст, который уже лежит внутри PDF. Мгновенно и без "
                "ошибок распознавания. На сканах и фотографиях откажет",
            },
            {
                "id": "vision",
                "label": "Распознать",
                "available": _engine_available("vision"),
                "description": "Для сканов, фотографий и снимков экрана. Распознаёт каждую "
                "страницу, даже если текст на ней уже есть",
            },
            {
                "id": "paddleocr",
                "label": "Другой движок",
                "available": _engine_available("paddleocr"),
                "description": "Второй распознаватель — на случай, если первый ошибся на "
                "конкретном файле. Заметно медленнее, ставится отдельно",
            },
        ],
    }
